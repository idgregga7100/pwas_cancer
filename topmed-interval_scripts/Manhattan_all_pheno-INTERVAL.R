...
#will need to merge by gene_name not ENSG label/PROBE_ID because INTERVAL results don't have ENSG names
        return(BP_Chrome)
}
...
format_files <- function(files, model_type, output){
	for(file in files){
		...
		if(str_detect(file, 'INTERVAL')){
			model<- 'INTERVAL'
			BP_Chrome<-load_bp_chrom('INTERVAL', model_type)
		}
...
#not gonna be able to merge with this so not needed i don't think
         	S_Pred_file <- S_Pred_file[-c( 9)]
      		...
      		GWAS <- merge(S_Pred_file, BP_Chrome, by = c('gene_name'))      
#      		GWAS <- S_Pred_file
		GWAS <- na.omit(GWAS)
      		names(GWAS)
...
#facet_wrap(~Model, nrow = 5)
		p <- ggplot(output, aes(x = BPcum, y = -log10(P), 
  			...
  			) + ggsave(paste(model_type, 'ALL-PAV-rho-Manhattan_total_phenotype_5e8.png', sep = ''),device='png', width = 12, height = 8)
