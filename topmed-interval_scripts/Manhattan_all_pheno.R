...
load_bp_chrom <- function(pop, type){
        print(paste("/home/isabelle/topmed/proteome/Schlidkraut/BP_Chrom_files/", pop, "_", type, "_BP_Chrome.csv", sep = ''))
        BP_Chrome <- read.table(paste("/home/isabelle/topmed/proteome/Schlidkraut/BP_Chrom_files/", pop, '_', type, "_BP_Chrome.csv", sep = ''), sep = ',', header = T)
...
get_files <- function(model_type){
	files <- list.files('/home/isabelle/topmed/proteome/Schlidkraut/SPred_out', pattern = model_type, recursive = T, full.names=T)
	return(files)
}
...
#		colorsss <- c("#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#276419", "#B15928", "#000000", "#8E0152")
		p <- ggplot(output, aes(x = BPcum, y = -log10(P), 
  			...
  			) + ggsave(paste(model_type, 'Manhattan_total_phenotype_5e8.png', sep = ''),device='tiff', width = 12, height = 8)
...
main <- function(){
	...
	pops <- c('CAU', 'ALL')
	#model_types <- c('PCAIR_baseline_models_PAV_filtered_rho0.1_zpval0.05', 'PCAIR_baseline_models_PAV_filtered_unfiltered', 'PCAIR_baseline_models_rho0.1_zpval0.05', 'PCAIR_baseline_models_unfiltered')
	model_types <- c('PCAIR_baseline_models_unfiltered')
	output <- data.frame()
	...
	for(model_type in model_types){
		...
		write.csv(output, paste('/home/isabelle/topmed/proteome/Schlidkraut/', model_type, '_sig_genes_all_pheno.csv', sep = ''), quote = F, row.names=F)
